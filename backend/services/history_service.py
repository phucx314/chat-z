...
def get_all_conversations(user_id: str) -> list:
    with SessionLocal() as db:
        convs = db.query(ConversationModel).filter(ConversationModel.user_id == user_id).order_by(ConversationModel.updated_at.desc()).all()
...
def get_conversation(conv_id: str, user_id: str) -> dict | None:
    with SessionLocal() as db:
        c = db.query(ConversationModel).filter(ConversationModel.id == conv_id, ConversationModel.user_id == user_id).first()
        if c:
...
def create_conversation(user_id: str, title: str = "New Chat") -> dict:
    with SessionLocal() as db:
        count = db.query(ConversationModel).filter(ConversationModel.user_id == user_id).count()
...
def update_conversation(conv_id: str, user_id: str, messages: list):
...
def update_avatar_color(conv_id: str, user_id: str, color: str):
    with SessionLocal() as db:
        c = db.query(ConversationModel).filter(ConversationModel.id == conv_id, ConversationModel.user_id == user_id).first()
        if c:
...
def delete_conversation(conv_id: str, user_id: str):
    with SessionLocal() as db:
        db.query(ConversationModel).filter(ConversationModel.id == conv_id, ConversationModel.user_id == user_id).delete()
        db.commit()

def delete_message(conv_id: str, user_id: str, message_index: int) -> list | None:
